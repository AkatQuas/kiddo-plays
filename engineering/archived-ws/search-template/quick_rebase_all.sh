...
Fetch and rebase in all story projects.
...
find $WORKSPACE -maxdepth 1 -type d -iname '*search-*' -not -name '*template*' -exec sh -c 'cd "$@" && make gfo' -- {} \;
# find $WORKSPACE -maxdepth 1 -type d -iname '*search-*' -not -name '*template*' -exec sh -c 'echo "hello $@" ' -- {} \;
