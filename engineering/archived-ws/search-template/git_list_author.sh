...
usage() {
  cat <<EOF
Usage: $(basename "${BASH_SOURCE[0]}") [-h] [-v] story repo

List commit authors in <story>/<repo>.
...
parse_params() {
  ...
  [[ ${#args[@]} -ne 2 ]] && die "Give me two arguments!"
...
msg "Listing author in ${YELLOW}$repo${NOFORMAT}"
