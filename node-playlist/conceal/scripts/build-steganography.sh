...
steganography_exec="$package_dir/steganography"
steganography_dir="$script_dir/steganography"
steganography_repo="git@github.com:7thSamurai/steganography.git"
...
Compile the dependency executable "steganography".
...
fetch_code() {
  if [ ! -d "$steganography_dir" ] ; then
    git clone --depth=1 "$steganography_repo" "$steganography_dir"
  fi
  return 0
}

build_once() {
  if [ -f "$steganography_exec" ]; then
    msg "${YELLOW}steganography exist, skipped${NOFORMAT}";
    msg "remove it before compiling a new one"
  else
    fetch_code
    cd "$steganography_dir"
    rm -rf build
    mkdir build
    cd build
    cmake -DCMAKE_BUILD_TYPE=Release ..
    make -j 4
    mv steganography "$steganography_exec"
    msg "${GREEN}steganography created${NOFORMAT}";
  fi
  return 0
}
