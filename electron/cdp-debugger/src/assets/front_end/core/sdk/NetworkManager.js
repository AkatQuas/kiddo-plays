...
export class NetworkManager extends SDKModel {
  ...
  constructor(target) {
    ...
    Common.Settings.Settings.instance().moduleSetting("cacheDisabled").addChangeListener(this.cacheDisabledSettingChanged, this);
  }
  ...
  static async requestContentData(request) {
    ...
    return { error, content: error ? null : response.body, encoded: response.base64Encoded };
  }
  ...
  dispose() {
    Common.Settings.Settings.instance().moduleSetting("cacheDisabled").removeChangeListener(this.cacheDisabledSettingChanged, this);
  }
...
export class NetworkDispatcher {
  ...
  signedExchangeReceived({ requestId, info }) {
    ...
    this.#manager.dispatchEventToListeners("ResponseReceived" /* ResponseReceived */, { request: networkRequest, response: info.outerResponse });
  }
  ...
  finishNetworkRequest(networkRequest, finishTime, encodedDataLength, shouldReportCorbBlocking) {
    ...
    if (shouldReportCorbBlocking) {
      ...
      this.#manager.dispatchEventToListeners("MessageGenerated" /* MessageGenerated */, { message, requestId: networkRequest.requestId(), warning: true });
    }
    if (Common.Settings.Settings.instance().moduleSetting("monitoringXHREnabled").get() && networkRequest.resourceType().category() === Common.ResourceType.resourceCategories.XHR) {
      let message;
      ...
      this.#manager.dispatchEventToListeners("MessageGenerated" /* MessageGenerated */, { message, requestId: networkRequest.requestId(), warning: false });
    }
  }
  clearRequests() {
...
((MultitargetNetworkManager2) => {
  let Events2;
  ((Events3) => {
    Events3["BlockedPatternsChanged"] = "BlockedPatternsChanged";
    Events3["ConditionsChanged"] = "ConditionsChanged";
    Events3["UserAgentChanged"] = "UserAgentChanged";
    Events3["InterceptorsChanged"] = "InterceptorsChanged";
    Events3["AcceptedEncodingsChanged"] = "AcceptedEncodingsChanged";
    Events3["RequestIntercepted"] = "RequestIntercepted";
...
export class InterceptedRequest {
  ...
  async continueRequestWithContent(contentBlob, encoded, responseHeaders) {
    ...
    async function blobToBase64(blob) {
      ...
      if (result === void 0 || result === null || typeof result !== "string") {
...
export function networkConditionsEqual(first, second) {
  ...
  return second.download === first.download && second.upload === first.upload && second.latency === first.latency && secondTitle === firstTitle;
}
