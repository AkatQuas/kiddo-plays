...
export class DOMNode {
  ...
  init(doc, isInShadowTree, payload) {
    ...
    const frameOwnerTags = /* @__PURE__ */ new Set(["EMBED", "IFRAME", "OBJECT", "PORTAL", "FENCEDFRAME"]);
  ...
  path() {
    function canPush(node2) {
      return (node2.index !== void 0 || node2.isShadowRoot() && node2.parentNode) && node2.#nodeNameInternal.length;
    }
    const path = [];
    let node = this;
    while (node && canPush(node)) {
      const index = typeof node.index === "number" ? node.index : node.shadowRootType() === DOMNode.ShadowRootTypes.UserAgent ? "u" : "a";
  ...
  async resolveToObject(objectGroup) {
    const { object } = await this.#agent.invoke_resolveNode({ nodeId: this.id, backendNodeId: void 0, objectGroup });
...
((DOMNode2) => {
  ...
  ((PseudoElementNames2) => {
    PseudoElementNames2["Before"] = "before";
    PseudoElementNames2["After"] = "after";
    PseudoElementNames2["Marker"] = "marker";
    PseudoElementNames2["PageTransition"] = "page-transition";
    PseudoElementNames2["PageTransitionContainer"] = "page-transition-container";
    PseudoElementNames2["PageTransitionImageWrapper"] = "page-transition-image-wrapper";
    PseudoElementNames2["PageTransitionOutgoingImage"] = "page-transition-outgoing-image";
    PseudoElementNames2["PageTransitionIncomingImage"] = "page-transition-incoming-image";
  })(PseudoElementNames = DOMNode2.PseudoElementNames || (DOMNode2.PseudoElementNames = {}));
  let ShadowRootTypes;
  ((ShadowRootTypes2) => {
...
export class DOMModel extends SDKModel {
  ...
  scheduleMutationEvent(node) {
    ...
    function callObserve(node2, mutationId) {
      ...
      this.dispatchEventToListeners(Events.DOMMutated, node2);
    }
  }
  requestDocument() {
    if (this.#document) {
