...
export class CSSProperty {
  ...
  async setText(propertyText, majorChange, overwrite) {
    ...
    const indentation = this.ownerStyle.cssText ? this.detectIndentation(this.ownerStyle.cssText) : Common.Settings.Settings.instance().moduleSetting("textEditorIndent").get();
  ...
  static async formatStyle(styleText, indentation, endIndentation) {
    ...
    function processToken(token, tokenType) {
      if (!insideProperty) {
        ...
        const isPropertyStart = tokenType?.includes("string") || tokenType?.includes("meta") || tokenType?.includes("property") || tokenType?.includes("variableName");
        if (disabledProperty) {
