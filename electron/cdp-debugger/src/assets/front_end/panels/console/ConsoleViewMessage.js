...
export class ConsoleViewMessage {
  ...
  formatAsNetworkRequest() {
    ...
    if (this.message.level === Protocol.Log.LogEntryLevel.Error) {
      ...
      const fragment = this.linkifyWithCustomLinkifier(messageText, (text, url, lineNumber, columnNumber) => {
        const linkElement = url === request.url() ? Components.Linkifier.Linkifier.linkifyRevealable(request, url, request.url()) : Components.Linkifier.Linkifier.linkifyURL(url, { text, lineNumber, columnNumber });
        linkElement.tabIndex = -1;
        this.selectableChildren.push({ element: linkElement, forceSelect: () => linkElement.focus() });
        return linkElement;
      });
  ...
  buildMessageWithStackTrace(runtimeModel) {
    ...
    const stackTracePreview = Components.JSPresentationUtils.buildStackTracePreviewContents(runtimeModel.target(), this.linkifier, { stackTrace: this.message.stackTrace, tabStops: void 0 });
  ...
  linkifyLocation(url, lineNumber, columnNumber) {
    ...
    return this.linkifier.linkifyScriptLocation(runtimeModel.target(), null, url, lineNumber, { columnNumber, inlineFrameIndex: 0 });
  }
  ...
  formatParameterAsObject(obj, includePreview) {
    ...
    if (includePreview && obj.preview) {
      ...
      this.previewFormatter.appendObjectPreview(titleElement, obj.preview, false);
  ...
  formatWithSubstitutionString(formatString, parameters, formattedResult) {
    ...
    for (const token of tokens) {
      switch (token.type) {
        case "generic": {
          formattedResult.append(this.formatParameter(token.value, true, false));
          break;
        }
        case "optimal": {
          formattedResult.append(this.formatParameter(token.value, false, true));
          break;
        }
        case "string": {
  ...
  maybeHandleOnKeyDown(event) {
    ...
    if (event.key === "ArrowUp") {
      ...
      if (this.selectNearestVisibleChild(focusedChildIndex - 1, true)) {
        return true;
      }
    }
  ...
  focusLastChildOrSelf() {
    if (this.elementInternal && !this.selectNearestVisibleChild(this.selectableChildren.length - 1, true)) {
  ...
  setSearchRegex(regex) {
    ...
    if (sourceRanges.length) {
      this.searchHighlightNodes = UI.UIUtils.highlightSearchResults(this.contentElement(), sourceRanges, this.searchHighlightNodeChanges);
    }
  }
  searchRegex() {
