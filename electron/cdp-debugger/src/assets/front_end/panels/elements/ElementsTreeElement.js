...
export class ElementsTreeElement extends UI.TreeOutline.TreeElement {
  ...
  startEditingAttribute(attribute, elementForSelection) {
    ...
    const attributeValue = attributeName && attributeValueElement ? this.nodeInternal.getAttribute(attributeName) : void 0;
    ...
    const config = new UI.InplaceEditor.Config(this.attributeEditingCommitted.bind(this), this.editingCancelled.bind(this), attributeName || void 0);
  ...
  buildAttributeDOM(parentElement, name, value, updateRecord, forceValue, node) {
    ...
    function setValueWithEntities(element, value2) {
      const result = this.convertWhitespaceToEntities(value2);
      highlightCount = result.entityRanges.length;
      value2 = result.text.replace(closingPunctuationRegex, (match, replaceOffset) => {
      ...
      element.setTextContentTruncatedIfNeeded(value2);
    ...
    function linkifyValue(value2) {
      const rewrittenHref = node ? node.resolveURL(value2) : null;
      if (rewrittenHref === null) {
        const span = document.createElement("span");
        setValueWithEntities.call(this, span, value2);
        return span;
      }
      value2 = value2.replace(closingPunctuationRegex, "$&\u200B");
      if (value2.startsWith("data:")) {
        value2 = Platform.StringUtilities.trimMiddle(value2, 60);
      }
      const link = node && node.nodeName().toLowerCase() === "a" ? UI.XLink.XLink.create(rewrittenHref, value2, "", true) : Components.Linkifier.Linkifier.linkifyURL(rewrittenHref, {
        text: value2,
        preventClick: true,
    ...
    function linkifySrcset(value2) {
      ...
      while (value2.length) {
        ...
        let url = "";
        let descriptor = "";
        const indexOfSpace = value2.search(/\s/);
        if (indexOfSpace === -1) {
          url = value2;
        } else if (indexOfSpace > 0 && value2[indexOfSpace - 1] === ",") {
          url = value2.substring(0, indexOfSpace);
        } else {
          url = value2.substring(0, indexOfSpace);
          const indexOfComma = value2.indexOf(",", indexOfSpace);
          if (indexOfComma !== -1) {
            descriptor = value2.substring(indexOfSpace, indexOfComma + 1);
          } else {
            descriptor = value2.substring(indexOfSpace);
          }
        }
        if (url) {
          if (url.endsWith(",")) {
        ...
        value2 = value2.substring(url.length + descriptor.length);
      }
      return fragment;
    }
