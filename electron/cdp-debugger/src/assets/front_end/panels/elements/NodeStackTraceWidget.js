...
export class NodeStackTraceWidget extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async doUpdate() {
    ...
    if (creationStackTrace) {
      ...
      const stackTracePreview = Components.JSPresentationUtils.buildStackTracePreviewContents(node.domModel().target(), this.linkifier, { stackTrace: creationStackTrace, tabStops: void 0 });
