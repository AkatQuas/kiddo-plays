...
export class BezierPopoverIcon {
  ...
  iconClick(event) {
    ...
    const cubicBezier = UI.Geometry.CubicBezier.parse(this.swatch.bezierText()) || UI.Geometry.CubicBezier.parse("linear");
    ...
    const uiLocation = Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().propertyUILocation(this.treeElement.property, false);
    if (uiLocation) {
      void Common.Revealer.reveal(uiLocation, true);
    }
  }
  bezierChanged(event) {
...
export class ColorSwatchPopoverIcon {
  ...
  spectrumChanged(event) {
    ...
    const colorName = this.spectrum ? this.spectrum.colorName() : void 0;
...
export class FontEditorSectionManager {
  ...
  async updateFontProperty(propertyName, value, treeElement) {
    if (treeElement && treeElement.treeOutline && treeElement.valueElement && treeElement.property.parsedOk && treeElement.property.range) {
