...
export class StylePropertiesSection {
  ...
  constructor(parentPane, matchedStyles, style, sectionIdx) {
    ...
    if (this.styleInternal.parentRule) {
      ...
      if (!this.newStyleRuleToolbar) {
        this.newStyleRuleToolbar = new UI.Toolbar.Toolbar("sidebar-pane-section-toolbar new-rule-toolbar", this.innerElement);
      }
    ...
    this.elementToSelectorIndex = /* @__PURE__ */ new WeakMap();
    if (rule) {
  ...
  resetToolbars() {
    if (this.parentPane.swatchPopoverHelper().isShowing() || this.styleInternal.type === SDK.CSSStyleDeclaration.Type.Inline) {
      return;
    }
    if (this.fontEditorToolbar) {
      this.fontEditorToolbar.element.classList.add("font-toolbar-hidden");
  ...
  static createRuleOriginNode(matchedStyles, linkifier, rule) {
    ...
    if (header?.isMutable && !header.isViaInspector()) {
      const location2 = header.isConstructedByNew() ? null : linkifyRuleLocation();
      if (location2) {
        return location2;
      }
  ...
  onNewRuleClick(event) {
    ...
    if (!rule || !rule.style.range || rule.styleSheetId === void 0) {
      return;
    }
  ...
  markSelectorMatches() {
    ...
    const fragment = this.hoverableSelectorsMode ? this.renderHoverableSelectors(selectorTexts, matchingSelectors) : this.renderSimplifiedSelectors(selectorTexts, matchingSelectors);
  ...
  handleEmptySpaceClick(event) {
    ...
    if (target.classList.contains("header") || this.element.classList.contains("read-only") || target.enclosingNodeOrSelfWithClass("query")) {
      event.consume();
      return;
    }
  ...
  handleQueryRuleClick(query, event) {
    ...
    const config = new UI.InplaceEditor.Config(this.editingMediaCommitted.bind(this, query), this.editingMediaCancelled.bind(this, element), void 0, this.editingMediaBlurHandler.bind(this));
  ...
  navigateToSelectorSource(index, focus) {
    ...
    const rule = this.styleInternal.parentRule;
    if (!rule || rule.styleSheetId === void 0) {
      return;
    }
...
export class BlankStylePropertiesSection extends StylePropertiesSection {
  ...
  constructor(stylesPane, matchedStyles, defaultSelectorText, styleSheetId, ruleLocation, insertAfterStyle, sectionIdx) {
    ...
    if (insertAfterStyle && insertAfterStyle.parentRule && insertAfterStyle.parentRule instanceof SDK.CSSRule.CSSStyleRule) {
