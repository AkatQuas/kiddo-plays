...
export class NetworkOverview extends PerfUI.TimelineOverviewPane.TimelineOverviewBase {
  ...
  update() {
    ...
    if (this.highlightedRequest) {
      ...
      const start = timeRanges[0].start * 1e3;
      const end = timeRanges[0].end * 1e3;
      ...
      for (let j = 0; j < timeRanges.length; ++j) {
        ...
        if (band !== -1 || type === RequestTimeRangeNames.Total) {
          ...
          const start2 = timeRanges[j].start * 1e3;
          const end2 = timeRanges[j].end * 1e3;
          context.moveTo(calculator.computePosition(start2) - 0, y);
          context.lineTo(calculator.computePosition(end2) + 1, y);
          context.stroke();
        }
      }
    }
