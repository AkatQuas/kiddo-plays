...
export class NetworkPanel extends UI.Panel.Panel {
  ...
  constructor() {
    super("network");
    this.networkLogShowOverviewSetting = Common.Settings.Settings.instance().createSetting("networkLogShowOverview", true);
    this.networkLogLargeRowsSetting = Common.Settings.Settings.instance().createSetting("networkLogLargeRows", false);
    this.networkRecordFilmStripSetting = Common.Settings.Settings.instance().createSetting("networkRecordFilmStripSetting", false);
    this.toggleRecordAction = UI.ActionRegistry.ActionRegistry.instance().action("network.toggle-recording");
    ...
    this.showSettingsPaneSetting = Common.Settings.Settings.instance().createSetting("networkShowSettingsToolbar", false);
  ...
  filmStripAvailable(filmStripModel) {
    ...
    if (this.filmStripView) {
      this.filmStripView.setModel(filmStripModel, calculator.minimumBoundary() * 1e3, calculator.boundarySpan() * 1e3);
    }
  ...
  appendApplicableItems(event, contextMenu, target) {
    function reveal(request2) {
      void UI.ViewManager.ViewManager.instance().showView("network").then(this.networkLogView.resetFilter.bind(this.networkLogView)).then(this.revealAndHighlightRequest.bind(this, request2));
    }
    function appendRevealItem(request2) {
      contextMenu.revealSection().appendItem(i18nString(UIStrings.revealInNetworkPanel), reveal.bind(this, request2));
    }
  ...
  onUpdateRequest(event) {
    ...
    this.overviewPane.setBounds(this.calculator.minimumBoundary() * 1e3, this.calculator.maximumBoundary() * 1e3);
...
export const displayScreenshotDelay = 1e3;
...
export class FilmStripRecorder {
  ...
  tracingComplete() {
    ...
    this.tracingManager = null;
    if (this.callback) {
      this.callback(new SDK.FilmStripModel.FilmStripModel(this.tracingModel, this.timeCalculator.minimumBoundary() * 1e3));
    }
    this.callback = null;
