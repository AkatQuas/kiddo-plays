...
export class NetworkConfigView extends UI.Widget.VBox {
  ...
  createAcceptedEncodingSection() {
    ...
    const customAcceptedEncodingSetting = Common.Settings.Settings.instance().createSetting("customAcceptedEncodings", `${Protocol.Network.ContentEncoding.Gzip},${Protocol.Network.ContentEncoding.Br},${Protocol.Network.ContentEncoding.Deflate}`);
    function onSettingChange() {
      if (!useCustomAcceptedEncodingSetting.get()) {
        ...
        SDK.NetworkManager.MultitargetNetworkManager.instance().setCustomAcceptedEncodingsOverride(customAcceptedEncodingSetting.get() === "" ? [] : customAcceptedEncodingSetting.get().split(","));
      }
    }
