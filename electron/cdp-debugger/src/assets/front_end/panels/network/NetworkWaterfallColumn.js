...
export class NetworkWaterfallColumn extends UI.Widget.VBox {
  ...
  static buildResourceTypeStyle() {
    ...
    for (const resourceType of Object.values(Common.ResourceType.resourceTypes)) {
      ...
      waitingStyleMap.set(resourceType, { fillStyle: toWaitingColor(color), lineWidth: 1, borderColor });
  ...
  getPopoverRequest(event) {
    ...
    const useTimingBars = !Common.Settings.Settings.instance().moduleSetting("networkColorCodeResourceTypes").get() && !this.calculator.startAtZero;
    let range;
    let start;
    let end;
    if (useTimingBars) {
      range = RequestTimingView.calculateRequestTimeRanges(request, 0).find((data) => data.name === RequestTimeRangeNames.Total);
  ...
  buildSimplifiedBarLayers(context, node, y) {
    ...
    if (!this.calculator.startAtZero) {
      const queueingRange = RequestTimingView.calculateRequestTimeRanges(request, 0).find((data) => data.name === RequestTimeRangeNames.Total);
  ...
  buildTimingBarLayers(node, y) {
    ...
    for (const range of ranges) {
      if (range.name === RequestTimeRangeNames.Total || range.name === RequestTimeRangeNames.Sending || range.end - range.start === 0) {
        continue;
      }
