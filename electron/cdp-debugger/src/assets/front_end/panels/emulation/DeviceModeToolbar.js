...
export class DeviceModeToolbar {
  ...
  constructor(model, showMediaInspectorSetting, showRulersSetting) {
    ...
    this.showDeviceScaleFactorSetting = Common.Settings.Settings.instance().createSetting("emulation.showDeviceScaleFactor", false);
    this.showDeviceScaleFactorSetting.addChangeListener(this.updateDeviceScaleFactorVisibility, this);
    this.showUserAgentTypeSetting = Common.Settings.Settings.instance().createSetting("emulation.showUserAgentType", false);
    ...
    this.heightInput = new EmulationComponents.DeviceSizeInputElement.SizeInputElement(i18nString(UIStrings.heightLeaveEmptyForFull));
    ...
    this.emulatedDevicesList.addEventListener(EmulationModel.EmulatedDevices.Events.CustomDevicesUpdated, this.deviceListChanged, this);
    this.emulatedDevicesList.addEventListener(EmulationModel.EmulatedDevices.Events.StandardDevicesUpdated, this.deviceListChanged, this);
  ...
  appendDeviceScaleMenuItems(contextMenu) {
    ...
    const defaultValue = this.model.uaSetting().get() === EmulationModel.DeviceModeModel.UA.Mobile || this.model.uaSetting().get() === EmulationModel.DeviceModeModel.UA.MobileNoTouch ? EmulationModel.DeviceModeModel.defaultMobileScaleFactor : window.devicePixelRatio;
  ...
  wrapToolbarItem(element) {
    ...
    const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(container, { cssFile: deviceModeToolbarStyles, delegatesFocus: void 0 });
  ...
  update() {
    ...
    if (this.experimentDualScreenSupport && this.experimentalButton) {
      const device2 = this.model.device();
      if (device2 && device2.isDualScreen) {
