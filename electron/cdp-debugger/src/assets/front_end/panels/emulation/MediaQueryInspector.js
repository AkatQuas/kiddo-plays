...
export class MediaQueryInspector extends UI.Widget.Widget {
  ...
  constructor(getWidthCallback, setWidthCallback) {
    ...
    UI.ZoomManager.ZoomManager.instance().addEventListener(UI.ZoomManager.Events.ZoomChanged, this.renderMediaQueries.bind(this), this);
  }
  modelAdded(cssModel) {
    if (this.cssModel) {
      return;
  ...
  onContextMenu(event) {
    ...
    for (let i = 0; i < locations.length; ++i) {
      ...
      const descriptor = typeof uiLocation.columnNumber === "number" ? Platform.StringUtilities.sprintf("%s:%d:%d", uiLocation.uiSourceCode.url(), uiLocation.lineNumber + 1, uiLocation.columnNumber + 1) : Platform.StringUtilities.sprintf("%s:%d", uiLocation.uiSourceCode.url(), uiLocation.lineNumber + 1);
...
export var Section = /* @__PURE__ */ ((Section2) => {
  ...
  Section2[Section2["MinMax"] = 1] = "MinMax";
