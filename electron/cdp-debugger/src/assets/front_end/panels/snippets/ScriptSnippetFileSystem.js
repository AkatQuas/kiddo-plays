...
export class SnippetFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  ...
  constructor() {
    ...
    this.lastSnippetIdentifierSetting = Common.Settings.Settings.instance().createSetting("scriptSnippets_lastIdentifier", 0);
...
//# sourceURL=${url}`,
  ...
  if ("exceptionDetails" in result && result.exceptionDetails) {
    SDK.ConsoleModel.ConsoleModel.instance().addMessage(SDK.ConsoleModel.ConsoleMessage.fromException(runtimeModel, result.exceptionDetails, void 0, void 0, url));
    return;
  }
  ...
  SDK.ConsoleModel.ConsoleModel.instance().addMessage(new SDK.ConsoleModel.ConsoleMessage(runtimeModel, Protocol.Log.LogEntrySource.Javascript, Protocol.Log.LogEntryLevel.Info, "", details));
}
