...
export class ThrottlingManager {
  ...
  createOfflineToolbarCheckbox() {
    ...
    checkbox.setChecked(SDK.NetworkManager.MultitargetNetworkManager.instance().networkConditions() === SDK.NetworkManager.OfflineConditions);
  ...
  updatePanelIcon() {
    ...
    if (cpuRate === SDK.CPUThrottlingManager.CPUThrottlingRates.NoThrottling && !this.hardwareConcurrencyOverrideEnabled) {
  ...
  createHardwareConcurrencySelector() {
    ...
    toggle.inputElement.disabled = true;
