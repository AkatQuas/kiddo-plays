...
export class AnimationUI {
  ...
  constructor(animation, timeline, parentElement) {
    ...
    UI.UIUtils.installDragHandle(this.#activeIntervalGroup, this.mouseDown.bind(this, Events.AnimationDrag, null), this.mouseMove.bind(this), this.mouseUp.bind(this), "-webkit-grabbing", "-webkit-grab");
    AnimationUI.installDragHandleKeyboard(this.#activeIntervalGroup, this.keydownMove.bind(this, Events.AnimationDrag, null));
    this.#cachedElements = [];
    this.#movementInMs = 0;
  ...
  drawDelayLine(parentElement) {
    ...
    const leftMargin = Math.min(this.#timeline.width(), (this.delay() + this.duration() * this.#animationInternal.source().iterations()) * this.#timeline.pixelMsRatio());
    this.#endDelayLine.style.transform = "translateX(" + leftMargin.toFixed(2) + "px)";
    this.#endDelayLine.setAttribute("x1", margin.toString());
    this.#endDelayLine.setAttribute("x2", forwardsFill ? (this.#timeline.width() - leftMargin + margin).toFixed(2) : (this.#animationInternal.source().endDelay() * this.#timeline.pixelMsRatio() + margin).toFixed(2));
  }
  drawPoint(iteration, parentElement, x, keyframeIndex, attachEvents) {
    ...
    circle.tabIndex = 0;
    UI.ARIAUtils.setAccessibleName(circle, keyframeIndex <= 0 ? i18nString(UIStrings.animationEndpointSlider) : i18nString(UIStrings.animationKeyframeSlider));
    if (keyframeIndex <= 0) {
  ...
  redraw() {
    ...
    this.#activeIntervalGroup.style.transform = "translateX(" + (this.delay() * this.#timeline.pixelMsRatio()).toFixed(2) + "px)";
    this.#nameElement.style.transform = "translateX(" + (this.delay() * this.#timeline.pixelMsRatio() + Options.AnimationMargin).toFixed(2) + "px)";
    ...
    let iteration;
    for (iteration = 1; iteration < this.#animationInternal.source().iterations() && iterationWidth * (iteration - 1) < this.#timeline.width() && (iterationWidth > 0 || this.#animationInternal.source().iterations() !== Infinity); iteration++) {
  ...
  renderIteration(parentElement, iteration) {
    ...
    group.style.transform = "translateX(" + (iteration * this.duration() * this.#timeline.pixelMsRatio()).toFixed(2) + "px)";
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["AnimationDrag"] = "AnimationDrag";
  Events2["KeyframeMove"] = "KeyframeMove";
  Events2["StartEndpointMove"] = "StartEndpointMove";
  Events2["FinishEndpointMove"] = "FinishEndpointMove";
  return Events2;
})(Events || {});
