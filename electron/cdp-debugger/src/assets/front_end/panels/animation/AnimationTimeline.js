...
const playbackRates = /* @__PURE__ */ new WeakMap();
...
export class AnimationTimeline extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#timelineControlsWidth = 150;
  ...
  createHeader() {
    ...
    this.#pauseButton = new UI.Toolbar.ToolbarToggle(i18nString(UIStrings.pauseAll), "largeicon-pause", "largeicon-resume");
    ...
    for (const playbackRate of GlobalPlaybackRates) {
      ...
      button.textContent = playbackRate ? i18nString(UIStrings.playbackRatePlaceholder, { PH1: playbackRate * 100 }) : i18nString(UIStrings.pause);
    ...
    this.#controlButton = new UI.Toolbar.ToolbarToggle(i18nString(UIStrings.replayTimeline), "largeicon-replay-animation");
  ...
  handlePlaybackRateControlKeyDown(event) {
    ...
    switch (keyboardEvent.key) {
      case "ArrowLeft":
      case "ArrowUp":
        this.focusNextPlaybackRateButton(event.target, true);
        break;
      case "ArrowRight":
      case "ArrowDown":
  ...
  getPopoverRequest(event) {
    ...
    return {
      ...
      show: (popover) => {
        ...
        return promise;
        function onFirstScreenshotLoaded(screenshots2) {
          new AnimationScreenshotPopover(screenshots2).show(popover.contentElement);
          fulfill(true);
        }
      },
      hide: void 0
  ...
  updateControlButton() {
    ...
    } else if (!this.#scrubberPlayer || !this.#scrubberPlayer.currentTime || this.#scrubberPlayer.currentTime >= this.duration()) {
      this.#controlState = ControlState.Replay;
      this.#controlButton.setToggled(true);
  ...
  handleAnimationGroupKeyDown(group, event) {
    switch (event.key) {
      ...
      case "ArrowLeft":
      case "ArrowUp":
        this.focusNextGroup(group, event.target, true);
        break;
      case "ArrowRight":
      case "ArrowDown":
        this.focusNextGroup(group, event.target);
    }
  }
  ...
  selectAnimationGroup(group) {
    function applySelectionClass(ui, group2) {
      ui.element.classList.toggle("selected", this.#selectedGroup === group2);
    }
    if (this.#selectedGroup === group) {
      this.togglePause(false);
      this.replay();
  ...
  renderGrid() {
    ...
    for (let time = 0; time < this.duration(); time += gridSize) {
      const gridWidth2 = time * this.pixelMsRatio();
      if (lastDraw === void 0 || gridWidth2 - lastDraw > 50) {
        lastDraw = gridWidth2;
        ...
        label.setAttribute("x", (gridWidth2 + 10).toString());
