...
export class ProfileSidebarTreeElement extends UI.TreeOutline.TreeElement {
  ...
  handleContextMenuEvent(event) {
    ...
    const sharedFileSelectorElement2 = getSharedFileSelectorElement();
    if (!sharedFileSelectorElement2) {
      throw new Error("File selector element shared by ProfilePanel instances is missing");
    }
    contextMenu.headerSection().appendItem(i18nString(UIStrings.load), sharedFileSelectorElement2.click.bind(sharedFileSelectorElement2));
    if (profile.canSaveToFile()) {
