...
export class HeapSnapshotView extends UI.View.SimpleView {
  ...
  constructor(dataDisplayDelegate, profile) {
    ...
    this.containmentDataGrid = new HeapSnapshotContainmentDataGrid(heapProfilerModel, this, i18nString(UIStrings.containment));
    ...
    this.perspectiveSelect = new UI.Toolbar.ToolbarComboBox(this.onSelectedPerspectiveChanged.bind(this), i18nString(UIStrings.perspective));
  ...
  createOverview() {
    ...
    if (!this.profile.fromFile() && profileType.profileBeingRecorded() === this.profile) {
      profileType.addEventListener(TrackingHeapSnapshotProfileTypeEvents.HeapStatsUpdate, this.onHeapStatsUpdate, this);
      profileType.addEventListener(TrackingHeapSnapshotProfileTypeEvents.TrackingStopped, this.onStopTracking, this);
      this.trackingOverviewGrid.start();
    }
  }
  onStopTracking() {
    const profileType = this.profile.profileType();
    profileType.removeEventListener(TrackingHeapSnapshotProfileTypeEvents.HeapStatsUpdate, this.onHeapStatsUpdate, this);
  ...
  changeFilter() {
    ...
    this.dataGrid.filterSelectIndexChanged(this.profiles(), profileIndex);
    if (!this.currentQuery || !this.searchResults) {
      return;
    }
    this.performSearch(this.currentQuery, false);
  ...
  onProfileHeaderRemoved(event) {
    ...
    if (this.profile === profile) {
      this.detach();
      this.profile.profileType().removeEventListener(HeapSnapshotProfileTypeEvents.SnapshotReceived, this.onReceiveSnapshot, this);
...
export class AllocationPerspective extends Perspective {
  ...
  constructor() {
    ...
    this.allocationSplitWidget = new UI.SplitWidget.SplitWidget(false, true, "heapSnapshotAllocationSplitViewState", 200, 200);
...
export var HeapSnapshotProfileTypeEvents = /* @__PURE__ */ ((HeapSnapshotProfileTypeEvents2) => {
  HeapSnapshotProfileTypeEvents2["SnapshotReceived"] = "SnapshotReceived";
...
export class TrackingHeapSnapshotProfileType extends Common.ObjectWrapper.eventMixin(HeapSnapshotProfileType) {
  ...
  constructor() {
    ...
    this.recordAllocationStacksSettingInternal = Common.Settings.Settings.instance().createSetting("recordAllocationStacks", false);
    this.customContentInternal = null;
...
export var TrackingHeapSnapshotProfileTypeEvents = /* @__PURE__ */ ((TrackingHeapSnapshotProfileTypeEvents2) => {
  TrackingHeapSnapshotProfileTypeEvents2["HeapStatsUpdate"] = "HeapStatsUpdate";
  TrackingHeapSnapshotProfileTypeEvents2["TrackingStarted"] = "TrackingStarted";
  TrackingHeapSnapshotProfileTypeEvents2["TrackingStopped"] = "TrackingStopped";
...
export class HeapProfileHeader extends ProfileHeader {
  ...
  saveToFile() {
    ...
    this.fileName = this.fileName || "Heap-" + Platform.DateUtilities.toISO8601Compact(new Date()) + this.profileType().fileExtension();
...
export class HeapSnapshotStatisticsView extends UI.Widget.VBox {
  ...
  static valueFormatter(value) {
    return i18nString(UIStrings.sKb, { PH1: Platform.NumberUtilities.withThousandsSeparator(Math.round(value / 1e3)) });
  }
