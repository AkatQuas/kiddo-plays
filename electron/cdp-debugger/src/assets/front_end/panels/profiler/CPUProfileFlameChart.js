...
export class ProfileFlameChartDataProvider {
  ...
  static colorGenerator() {
    if (!colorGeneratorInstance) {
      colorGeneratorInstance = new Common.Color.Generator({ min: 30, max: 330, count: void 0 }, { min: 50, max: 80, count: 5 }, { min: 80, max: 90, count: 3 });
...
export class CPUProfileFlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  onWindowChanged(event) {
    ...
    this.mainPane.setWindowTimes(windowLeft, windowRight, true);
  }
...
export var OverviewPaneEvents = /* @__PURE__ */ ((OverviewPaneEvents2) => {
  OverviewPaneEvents2["WindowChanged"] = "WindowChanged";
