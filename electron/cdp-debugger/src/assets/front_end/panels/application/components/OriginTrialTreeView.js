...
export class OriginTrialTokenRows extends HTMLElement {
  ...
  #renderTokenField = (fieldValue, hasError) => LitHtml.html`
        <div class=${LitHtml.Directives.ifDefined(hasError ? "error-text" : void 0)}>
          ${fieldValue}
        </div>`;
  #setTokenFields() {
    ...
    this.#parsedTokenDetails = [
      {
        ...
        value: this.#renderTokenField(this.#tokenWithStatus.parsedToken.origin, this.#tokenWithStatus.status === Protocol.Page.OriginTrialTokenStatus.WrongOrigin)
      },
      {
        name: i18nString(UIStrings.expiryTime),
        value: this.#renderTokenField(this.#dateFormatter.format(this.#tokenWithStatus.parsedToken.expiryTime * 1e3), this.#tokenWithStatus.status === Protocol.Page.OriginTrialTokenStatus.Expired)
      },
      {
