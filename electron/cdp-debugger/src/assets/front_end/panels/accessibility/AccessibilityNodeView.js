...
export class AXNodePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  appendRelatedNodeInline(relatedNode) {
    const deferredNode = new SDK.DOMModel.DeferredDOMNode(this.axNode.accessibilityModel().target(), relatedNode.backendDOMNodeId);
    const linkedNode = new AXRelatedNodeElement({ deferredNode, idref: void 0 }, relatedNode);
...
export class AXValueSourceTreeElement extends AXNodePropertyTreeElement {
  ...
  update() {
    ...
    if (this.source.attributeValue) {
      ...
      const valueElement = AXNodePropertyTreeElement.createSimpleValueElement(Protocol.Accessibility.AXValueType.ValueUndefined, i18nString(UIStrings.notSpecified));
...
export class AXRelatedNodeSourceTreeElement extends UI.TreeOutline.TreeElement {
  ...
  onattach() {
    ...
    if (this.value.text) {
      this.listItemElement.appendChild(AXNodePropertyTreeElement.createSimpleValueElement(Protocol.Accessibility.AXValueType.ComputedString, this.value.text));
    }
  }
  onenter() {
  ...
  static createReasonElement(reason, axNode) {
    ...
    switch (reason) {
      ...
      case "presentationalRole": {
        ...
        reasonElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.elementHasPlaceholder, { PH1: rolePresentationSpan });
        break;
      }
