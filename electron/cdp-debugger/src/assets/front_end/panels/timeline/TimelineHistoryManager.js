...
export class TimelineHistoryManager {
  ...
  constructor() {
    this.recordings = [];
    this.action = UI.ActionRegistry.ActionRegistry.instance().action("timeline.show-history");
  ...
  static previewElement(performanceModel) {
    ...
    data.time.textContent = startedAt ? i18nString(UIStrings.sAgo, { PH1: TimelineHistoryManager.coarseAge(startedAt) }) : "";
    return data.preview;
  }
  static coarseAge(time) {
    const seconds = Math.round((Date.now() - time) / 1e3);
    if (seconds < 50) {
  ...
  buildPreview(performanceModel) {
    ...
    const data = { preview, title, time: timeElement, lastUsed: Date.now() };
  ...
  buildScreenshotThumbnail(performanceModel) {
    ...
    void lastFrame.imageDataPromise().then((data) => UI.UIUtils.loadImageFromData(data)).then((image) => image && container.appendChild(image));
    return container;
  }
...
const modelToPerformanceData = /* @__PURE__ */ new WeakMap();
