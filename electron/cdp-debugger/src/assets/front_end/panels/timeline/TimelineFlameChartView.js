...
class MainSplitWidget extends UI.SplitWidget.SplitWidget {
  ...
  setModelAndUpdateBoundaries(model) {
    ...
    const longTasks = events.filter((e) => SDK.TracingModel.TracingModel.isCompletePhase(e.phase) && timelineModel.isLongRunningTask(e)).map((e) => ({ start: e.startTime - minimumBoundary, duration: e.duration || 0 }));
...
export class TimelineFlameChartView extends UI.Widget.VBox {
  ...
  constructor(delegate) {
    ...
    this.networkFlameChartGroupExpansionSetting = Common.Settings.Settings.instance().createSetting("timelineFlamechartNetworkViewGroupExpansion", {});
    this.networkDataProvider = new TimelineFlameChartNetworkDataProvider();
    this.networkFlameChart = new PerfUI.FlameChart.FlameChart(this.networkDataProvider, this, this.networkFlameChartGroupExpansionSetting);
    ...
    this.mainSplitWidget = new MainSplitWidget(false, false, "timelineFlamechartMainAndVitalsView", void 0, 120);
  ...
  updateColorMapper() {
    this.urlToColorCache = /* @__PURE__ */ new Map();
    if (!this.model) {
      return;
    }
    this.mainDataProvider.setEventColorMapping(TimelineUIUtils.eventColor);
  ...
  resizeToPreferredHeights() {
    ...
    this.networkSplitWidget.setSidebarSize(this.networkDataProvider.preferredHeight() + this.splitResizer.clientHeight + PerfUI.FlameChart.HeaderHeight + 2);
  }
