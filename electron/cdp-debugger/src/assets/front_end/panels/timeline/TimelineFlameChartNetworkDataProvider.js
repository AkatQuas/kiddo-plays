...
export class TimelineFlameChartNetworkDataProvider {
  ...
  constructor() {
    ...
    this.group = { startLevel: 0, name: i18nString(UIStrings.network), expanded: false, style: this.style };
  ...
  decorateEntry(index, context, text, barX, barY, barWidth, barHeight, unclippedBarX, timeToPixelRatio) {
    ...
    if (!request.cached() && timing.pushStart) {
      const pushStart = timeToPixel(timing.pushStart * 1e3);
      const pushEnd = timing.pushEnd ? timeToPixel(timing.pushEnd * 1e3) : start;
  ...
  prepareHighlightedEntryInfo(index) {
    ...
    if (startTime && isFinite(duration)) {
      contents.createChild("span", "timeline-info-network-time").textContent = i18n.TimeUtilities.millisToString(duration, true);
    }
    if (typeof request.priority === "string") {
      const div = contents.createChild("span");
      div.textContent = PerfUI.NetworkPriorities.uiLabelForNetworkPriority(request.priority);
  ...
  colorForPriority(priority) {
    if (!this.priorityToValue) {
      this.priorityToValue = /* @__PURE__ */ new Map([
  ...
  appendTimelineData() {
    if (this.model) {
      ...
      this.timeSpan = this.model.isEmpty() ? 1e3 : this.maximumBoundary - this.minimumBoundaryInternal;
