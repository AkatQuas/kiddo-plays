...
export class TimelineFlameChartDataProvider extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.collapsibleTimingsHeader = this.buildGroupStyle({ shareHeaderLine: true, useFirstLineForOverview: true, collapsible: true });
    this.timingsHeader = this.buildGroupStyle({ shareHeaderLine: true, useFirstLineForOverview: true, collapsible: false });
    this.screenshotsHeader = this.buildGroupStyle({ useFirstLineForOverview: true, nestingLevel: 1, collapsible: false, itemsHeight: 150 });
    ...
    this.flowEventIndexById = /* @__PURE__ */ new Map();
  }
  ...
  reset() {
    ...
    this.markers = [];
    this.asyncColorByCategory = /* @__PURE__ */ new Map();
    this.asyncColorByInteractionPhase = /* @__PURE__ */ new Map();
    this.extensionInfo = [];
    this.screenshotImageCache = /* @__PURE__ */ new Map();
  }
  maxStackDepth() {
  ...
  timelineData() {
    ...
    this.timeSpan = this.model.isEmpty() ? 1e3 : this.model.maximumRecordTime() - this.minimumBoundaryInternal;
    this.currentLevel = 0;
  ...
  processGenericTrace() {
    ...
    for (const process of tracksByProcess.keysArray()) {
      if (tracksByProcess.size > 1) {
        ...
        this.appendHeader(name, processGroupStyle, false);
      }
      for (const track of tracksByProcess.get(process)) {
        const group = this.appendSyncEvents(track, track.events, track.name, threadGroupStyle, eventEntryType, true);
        if (this.timelineDataInternal && (!this.timelineDataInternal.selectedGroup || track.name === TimelineModel.TimelineModel.TimelineModelImpl.BrowserMainThreadName)) {
  ...
  processInspectorTrace() {
    ...
    for (const track of tracks) {
      switch (track.type) {
        case TimelineModel.TimelineModel.TrackType.Animation: {
          this.appendAsyncEventsGroup(track, i18nString(UIStrings.animation), track.asyncEvents, this.animationsHeader, eventEntryType, false);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Timings: {
          const style = track.asyncEvents.length > 0 ? this.collapsibleTimingsHeader : this.timingsHeader;
          const group = this.appendHeader(i18nString(UIStrings.timings), style, true);
          group.track = track;
          this.appendPageMetrics();
          this.copyPerfMarkEvents(track);
          this.appendSyncEvents(track, track.events, null, null, eventEntryType, true);
          this.appendAsyncEventsGroup(track, null, track.asyncEvents, null, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Console: {
          this.appendAsyncEventsGroup(track, i18nString(UIStrings.console), track.asyncEvents, this.headerLevel1, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.MainThread: {
          if (track.forMainFrame) {
            const group = this.appendSyncEvents(track, track.events, track.url ? i18nString(UIStrings.mainS, { PH1: track.url }) : i18nString(UIStrings.main), this.headerLevel1, eventEntryType, true);
            if (group && this.timelineDataInternal) {
              this.timelineDataInternal.selectedGroup = group;
            }
          } else {
            this.appendSyncEvents(track, track.events, track.url ? i18nString(UIStrings.frameS, { PH1: track.url }) : i18nString(UIStrings.subframe), this.headerLevel1, eventEntryType, true);
          }
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Worker: {
          this.appendSyncEvents(track, track.events, track.name, this.headerLevel1, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Raster: {
          if (!rasterCount) {
            this.appendHeader(i18nString(UIStrings.raster), this.headerLevel1, false);
          }
          ++rasterCount;
          this.appendSyncEvents(track, track.events, i18nString(UIStrings.rasterizerThreadS, { PH1: rasterCount }), this.headerLevel2, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.GPU: {
          this.appendSyncEvents(track, track.events, i18nString(UIStrings.gpu), this.headerLevel1, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Other: {
          this.appendSyncEvents(track, track.events, track.name || i18nString(UIStrings.thread), this.headerLevel1, eventEntryType, true);
          this.appendAsyncEventsGroup(track, track.name, track.asyncEvents, this.headerLevel1, eventEntryType, true);
          break;
        }
        case TimelineModel.TimelineModel.TrackType.Experience: {
          this.appendSyncEvents(track, track.events, i18nString(UIStrings.experience), this.experienceHeader, eventEntryType, true);
          break;
        }
      }
    }
  ...
  appendSyncEvents(track, events, title, style, entryType, selectable) {
    ...
    for (let i = 0; i < events.length; ++i) {
      ...
      while (openEvents.length && openEvents[openEvents.length - 1].endTime <= e.startTime) {
        openEvents.pop();
      }
      ...
      if (!isExtension && this.performanceModel.timelineModel().isMarkerEvent(e)) {
        this.timelineDataInternal.entryTotalTimes[this.entryData.length] = void 0;
      }
  ...
  appendPageMetrics() {
    ...
    if (lcpEvents.length > 0) {
      const lcpEventsByNavigationId = /* @__PURE__ */ new Map();
  ...
  appendFrames() {
    ...
    this.appendHeader("", this.screenshotsHeader, false);
    ...
    if (screenshots.length && prevTimestamp !== void 0) {
  ...
  prepareHighlightedEntryInfo(entryIndex) {
    ...
    if (type === EntryType.Event) {
      ...
      if (typeof totalTime === "number") {
        time = Math.abs(totalTime - selfTime) > eps && selfTime > eps ? i18nString(UIStrings.sSelfS, {
  ...
  entryColor(entryIndex) {
    ...
    if (type === entryTypes.Event) {
      ...
      if (event.hasCategory(TimelineModel.TimelineModel.TimelineModelImpl.Category.LatencyInfo)) {
        const phase = TimelineModel.TimelineIRModel.TimelineIRModel.phaseForEvent(event) || TimelineModel.TimelineIRModel.Phases.Uncategorized;
  ...
  async drawScreenshot(entryIndex, context, barX, barY, barWidth, barHeight) {
    ...
    if (!this.screenshotImageCache.has(screenshot)) {
      ...
      const image2 = await UI.UIUtils.loadImageFromData(data);
      this.screenshotImageCache.set(screenshot, image2);
  ...
  innerAppendExtensionEvents(index) {
    ...
    if (!singleTrack) {
      this.appendHeader(entry.title, this.headerLevel1, false);
    }
    ...
    for (const thread of allThreads) {
      ...
      this.appendAsyncEventsGroup(null, title, thread.asyncEvents(), style, entryType, false);
      this.appendSyncEvents(null, thread.events(), title, style, entryType, false);
    }
  }
  appendHeader(title, style, selectable) {
    const group = { startLevel: this.currentLevel, name: title, style, selectable };
  ...
  eventByIndex(entryIndex) {
    return entryIndex >= 0 && this.entryType(entryIndex) === EntryType.Event ? this.entryData[entryIndex] : null;
  }
...
export const InstantEventVisibleDurationMs = 1e-3;
const eventToDisallowRoot = /* @__PURE__ */ new WeakMap();
const indexForEvent = /* @__PURE__ */ new WeakMap();
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["DataChanged"] = "DataChanged";
  return Events2;
})(Events || {});
export var EntryType = /* @__PURE__ */ ((EntryType2) => {
  EntryType2["Frame"] = "Frame";
  EntryType2["Event"] = "Event";
  EntryType2["InteractionRecord"] = "InteractionRecord";
  EntryType2["ExtensionEvent"] = "ExtensionEvent";
