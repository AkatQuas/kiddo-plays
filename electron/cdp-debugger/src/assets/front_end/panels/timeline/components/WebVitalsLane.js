...
export class WebVitalsEventLane extends WebVitalsLane {
  ...
  #renderLabel(position, label, textMetrics) {
    ...
    this.context.fillText(label, this.tX(position) + this.timeline.getLineHeight() * 0.5, 0.5 * this.timeline.getLineHeight() + height * 0.5);
    this.context.restore();
  }
  #renderTimestamp(position, textWidth, timestamp, textMetrics) {
    ...
    const height = textMetrics.actualBoundingBoxAscent - textMetrics.actualBoundingBoxDescent;
    this.context.fillStyle = this.theme.textSecondary;
    this.context.fillText(timestamp, this.tX(position) + this.timeline.getLineHeight() * 0.5 + textWidth + 5, 0.5 * this.timeline.getLineHeight() + height * 0.5);
    this.context.restore();
  }
