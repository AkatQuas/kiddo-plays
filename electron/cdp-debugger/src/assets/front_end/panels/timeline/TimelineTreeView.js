...
export class TimelineTreeView extends UI.Widget.VBox {
  ...
  init() {
    ...
    this.taskFilter = new TimelineModel.TimelineModelFilter.ExclusiveNameFilter([TimelineModel.TimelineModel.RecordType.Task]);
  ...
  selectProfileNode(treeNode, suppressSelectedEvent) {
    ...
    for (let i = pathToRoot.length - 1; i > 0; --i) {
      ...
      if (gridNode2 && gridNode2.dataGrid) {
        gridNode2.expand();
      }
    }
...
export class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
  ...
  createNameCell(columnId) {
    ...
    if (this.profileNode.isGroupNode()) {
      ...
      if (deoptReason) {
        container.createChild("div", "activity-warning").title = i18nString(UIStrings.notOptimizedS, { PH1: deoptReason });
      }
...
const profileNodeToTreeGridNode = /* @__PURE__ */ new WeakMap();
export class AggregatedTimelineTreeView extends TimelineTreeView {
  ...
  constructor() {
    ...
    this.productByURLCache = /* @__PURE__ */ new Map();
    this.colorByURLCache = /* @__PURE__ */ new Map();
    this.executionContextNamesByOrigin = /* @__PURE__ */ new Map();
  }
  setModel(model, track) {
  ...
  displayInfoForGroupNode(node) {
    ...
    switch (this.groupBySetting.get()) {
      ...
      case AggregatedTimelineTreeView.GroupBy.Subdomain: {
        const domainName = id ? this.beautifyDomainName(id) : void 0;
      ...
      case AggregatedTimelineTreeView.GroupBy.EventName: {
        ...
        const name = node.event.name === TimelineModel.TimelineModel.RecordType.JSFrame ? i18nString(UIStrings.javascript) : TimelineUIUtils.eventTitle(node.event);
        return {
          name,
          color: node.event.name === TimelineModel.TimelineModel.RecordType.JSFrame ? TimelineUIUtils.eventStyle(node.event).category.color : color,
          icon: void 0
        };
      }
      ...
      case AggregatedTimelineTreeView.GroupBy.Frame: {
        ...
        const frame = id ? this.modelInternal.timelineModel().pageFrameById(id) : void 0;
...
((TimelineStackView2) => {
  let Events;
  ((Events2) => {
    Events2["SelectionChanged"] = "SelectionChanged";
