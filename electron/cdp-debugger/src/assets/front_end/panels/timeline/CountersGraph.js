...
export class CountersGraph extends UI.Widget.VBox {
  ...
  onWindowChanged(event) {
    ...
    this.calculator.setWindow(window2.left, window2.right);
    this.scheduleRefresh();
  }
  scheduleRefresh() {
...
export class CounterUI {
  ...
  recordIndexAt(x) {
    return Platform.ArrayUtilities.upperBound(this.counter.x, x * window.devicePixelRatio, Platform.ArrayUtilities.DEFAULT_COMPARATOR, this.counter.minimumIndex + 1, this.counter.maximumIndex + 1) - 1;
  }
