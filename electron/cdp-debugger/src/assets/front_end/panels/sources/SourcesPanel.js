...
export class SourcesPanel extends UI.Panel.Panel {
  ...
  constructor() {
    ...
    this.togglePauseAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.toggle-pause");
    this.stepOverAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.step-over");
    this.stepIntoAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.step-into");
    this.stepOutAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.step-out");
    this.stepAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.step");
    this.toggleBreakpointsActiveAction = UI.ActionRegistry.ActionRegistry.instance().action("debugger.toggle-breakpoints-active");
    ...
    this.splitWidget = new UI.SplitWidget.SplitWidget(true, true, "sourcesPanelSplitViewState", initialDebugSidebarWidth);
    this.splitWidget.enableShowModeSaving();
    this.splitWidget.show(this.element);
    const initialNavigatorWidth = 225;
    this.editorView = new UI.SplitWidget.SplitWidget(true, false, "sourcesPanelNavigatorSplitViewState", initialNavigatorWidth);
    ...
    Common.Settings.Settings.instance().moduleSetting("pauseOnExceptionEnabled").addChangeListener(this.pauseOnExceptionEnabledChanged, this);
    ...
    Common.Settings.Settings.instance().moduleSetting("breakpointsActive").addChangeListener(this.breakpointsActiveStateChanged, this);
  ...
  debuggerPaused(event) {
    ...
    if (!this.pausedInternal && Common.Settings.Settings.instance().moduleSetting("autoFocusOnDebuggerPausedEnabled").get()) {
  ...
  showUISourceCode(uiSourceCode, lineNumber, columnNumber, omitFocus) {
    ...
    this.sourcesViewInternal.showSourceLocation(uiSourceCode, lineNumber === void 0 ? void 0 : { lineNumber, columnNumber }, omitFocus);
  }
  showEditor() {
    if (WrapperView.isShowing()) {
      return;
  ...
  async callFrameChanged() {
    ...
    this.executionLineLocation = await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().createCallFrameLiveLocation(callFrame.location(), this.executionLineChanged.bind(this), this.liveLocationPool);
  }
  ...
  togglePauseOnExceptions() {
    Common.Settings.Settings.instance().moduleSetting("pauseOnExceptionEnabled").set(!this.pauseOnExceptionButton.toggled());
  }
  runSnippet() {
  ...
  editorSelected(event) {
    ...
    if (this.editorView.mainWidget() && Common.Settings.Settings.instance().moduleSetting("autoRevealInNavigator").get()) {
  ...
  toggleBreakpointsActive() {
    Common.Settings.Settings.instance().moduleSetting("breakpointsActive").set(!Common.Settings.Settings.instance().moduleSetting("breakpointsActive").get());
  }
  ...
  appendUISourceCodeItems(event, contextMenu, target) {
    ...
    if (!uiSourceCode.project().isServiceProject() && !eventTarget.isSelfOrDescendant(this.navigatorTabbedLocation.widget().element)) {
  ...
  updateSidebarPosition() {
    ...
    this.sidebarPaneStack = UI.ViewManager.ViewManager.instance().createStackLocation(this.revealDebuggerSidebar.bind(this));
...
export class UISourceCodeRevealer {
  ...
  async reveal(uiSourceCode, omitFocus) {
    ...
    SourcesPanel.instance().showUISourceCode(uiSourceCode, void 0, void 0, omitFocus);
  }
}
  ...
  handleAction(context, actionId) {
    ...
    switch (actionId) {
      case "debugger.toggle-pause": {
        const actionHandledInPausedOverlay = context.flavor(UI.ShortcutRegistry.ForwardedShortcut) && !Common.Settings.Settings.instance().moduleSetting("disablePausedStateOverlay").get();
      ...
      case "debugger.evaluate-selection": {
        ...
        if (frame) {
          ...
          if (executionContext) {
            ...
            void SDK.ConsoleModel.ConsoleModel.instance().evaluateCommandInConsole(executionContext, message, text, true);
          }
        }
        return true;
      }
