...
export class NavigatorView extends UI.Widget.VBox {
  ...
  projectAdded(project) {
    ...
    if (!this.acceptProject(project) || project.type() !== Workspace.Workspace.projectTypes.FileSystem || Snippets.ScriptSnippetFileSystem.isSnippetsProject(project) || rootOrDeployed.child(project.id())) {
      return;
    }
  ...
  computeUniqueFileSystemProjectNames() {
    ...
    for (let i = 0; i < fileSystemProjects.length; ++i) {
      ...
      const commonPrefix = reversedIndex.longestPrefix(reversedPath, false);
      reversedIndex.add(reversedPath);
  ...
  removeUISourceCodeNode(node) {
    ...
    while (currentNode) {
      ...
      const folderId = this.folderNodeId(project, target, frame, uiSourceCode.origin(), uiSourceCode.contentType().isFromSourceMap(), currentNode instanceof NavigatorFolderTreeNode && currentNode.folderPath || Platform.DevToolsPath.EmptyEncodedPathString);
...
export class NavigatorFolderTreeElement extends UI.TreeOutline.TreeElement {
  ...
  constructor(navigatorView, type, title, hoverCallback) {
    ...
    if (legacyIcon) {
      ...
      icon.data = { iconPath, color: "var(--override-folder-tree-item-color)", width: "18px" };
