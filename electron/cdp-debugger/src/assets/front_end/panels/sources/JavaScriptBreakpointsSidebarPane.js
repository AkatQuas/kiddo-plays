...
export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async doUpdate() {
    ...
    for (let idx = 0; idx < locationsGroupedById.length; idx++) {
      ...
      const uiLocation = breakpointLocation.uiLocation;
      const isSelected = selectedUILocation !== null && locations.some((location) => location.uiLocation.id() === selectedUILocation.id());
      const showColumn = uiLocation.uiSourceCode.mimeType() !== "application/wasm" && locationIdsByLineId.get(uiLocation.lineId()).size > 1;
    ...
    if (this.breakpoints.length === breakpointItems.length) {
      for (let i = 0; i < this.breakpoints.length; i++) {
        if (!this.breakpoints.at(i).isSimilar(breakpointItems[i])) {
          this.breakpoints.replace(i, breakpointItems[i], true);
        }
      }
    } else {
  ...
  createElementForItem(item) {
    ...
    checkboxLabel.textElement.textContent = uiLocation.linkText() + (item.showColumn && typeof uiLocation.columnNumber === "number" ? ":" + (uiLocation.columnNumber + 1) : "");
  ...
  breakpointContextMenu(event) {
    ...
    const removeEntryTitle = breakpoints.length > 1 ? i18nString(UIStrings.removeAllBreakpointsInLine) : i18nString(UIStrings.removeBreakpoint);
    contextMenu.defaultSection().appendItem(removeEntryTitle, () => breakpoints.map((breakpoint) => void breakpoint.remove(false)));
    if (event.target instanceof Element) {
  ...
  removeAllBreakpoints() {
    for (const breakpointLocation of this.breakpointManager.allBreakpointLocations()) {
      void breakpointLocation.breakpoint.remove(false);
    }
  }
...
const elementToUILocationMap = /* @__PURE__ */ new WeakMap();
export function retrieveLocationForElement(element) {
  return elementToUILocationMap.get(element);
}
const elementToBreakpointMap = /* @__PURE__ */ new WeakMap();
