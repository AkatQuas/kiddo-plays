...
export class BreakpointEditDialog extends UI.Widget.Widget {
  ...
  constructor(editorLineNumber, oldCondition, preferLogpoint, onFinish) {
    ...
    this.typeSelector = new UI.Toolbar.ToolbarComboBox(this.onTypeChanged.bind(this), i18nString(UIStrings.breakpointType));
    ...
    this.element.addEventListener("blur", (event) => {
      if (!event.relatedTarget || event.relatedTarget && !event.relatedTarget.isSelfOrDescendant(this.element)) {
        this.finishEditing(true, this.editor.state.doc.toString());
      }
    }, true);
  }
