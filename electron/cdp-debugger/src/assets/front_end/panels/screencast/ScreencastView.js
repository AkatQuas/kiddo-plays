...
export class ScreencastView extends UI.Widget.VBox {
  ...
  startCasting() {
    ...
    this.screenCaptureModel.startScreencast(Protocol.Page.StartScreencastRequestFormat.Jpeg, 80, Math.floor(Math.min(maxImageDimension, dimensions.width)), Math.floor(Math.min(maxImageDimension, dimensions.height)), void 0, this.screencastFrame.bind(this), this.screencastVisibilityChanged.bind(this));
  ...
  cssColor(color) {
    ...
    return Common.Color.Color.fromRGBA([color.r, color.g, color.b, color.a !== void 0 ? color.a : 1]).asString(Common.Color.Format.RGBA) || "";
  }
  quadToPath(quad) {
