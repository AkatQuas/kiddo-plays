...
export class InspectorMainImpl {
  ...
  async run() {
    ...
    await SDK.Connections.initMainConnection(async () => {
      ...
      const target = SDK.TargetManager.TargetManager.instance().createTarget("main", i18nString(UIStrings.main), type, null, void 0, waitForDebuggerInPage);
      if (!firstCall) {
        return;
      }
      firstCall = false;
...
export class NodeIndicator {
  ...
  constructor() {
    ...
    const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(element, { cssFile: [nodeIconStyles], delegatesFocus: void 0 });
...
export class SourcesPanelIndicator {
  constructor() {
    Common.Settings.Settings.instance().moduleSetting("javaScriptDisabled").addChangeListener(javaScriptDisabledChanged);
