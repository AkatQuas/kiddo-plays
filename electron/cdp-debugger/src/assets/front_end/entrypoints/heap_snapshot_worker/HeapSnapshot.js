...
export class HeapSnapshot {
  ...
  calculateDistances(filter) {
    ...
    distances[this.rootNode().ordinal()] = nodesToVisitLength > 0 ? HeapSnapshotModel.HeapSnapshotModel.baseSystemDistance : 0;
  ...
  calculateClassesRetainedSize(aggregates, filter) {
    ...
    const sizes = [-1];
    const classes = [];
    const seenClassNameIndexes = /* @__PURE__ */ new Map();
    const nodeFieldCount = this.nodeFieldCount;
    const nodeTypeOffset = this.nodeTypeOffset;
    const nodeNativeType = this.nodeNativeType;
    ...
    while (list.length) {
      ...
      if (!seen && (!filter || filter(node)) && (node.selfSize() || nodes[nodeIndex + nodeTypeOffset] === nodeNativeType)) {
...
export class HeapSnapshotEdgesProvider extends HeapSnapshotItemProvider {
  ...
  sort(comparator, leftBound, rightBound, windowLeft, windowRight) {
    ...
    function compareEdgeFieldName(ascending, indexA, indexB) {
      ...
      const result = edgeA.hasStringName() === edgeB.hasStringName() ? edgeA.name() < edgeB.name() ? -1 : edgeA.name() > edgeB.name() ? 1 : 0 : edgeA.hasStringName() ? -1 : 1;
