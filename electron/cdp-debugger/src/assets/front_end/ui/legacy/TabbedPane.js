...
export class TabbedPane extends Common.ObjectWrapper.eventMixin(VBox) {
  ...
  adjustToolbarWidth() {
    ...
    } else if (this.rightToolbarInternal.hasCompactLayout() && totalWidth - 2 * rightToolbarWidth - leftToolbarWidth > this.measuredDropDownButtonWidth + 10) {
  ...
  dropDownClicked(ev) {
    ...
    for (const tab of this.tabs) {
      ...
      if (this.numberOfTabsShown() === 0 && this.tabsHistory[0] === tab) {
        menu.defaultSection().appendCheckboxItem(tab.title, this.dropDownMenuItemSelected.bind(this, tab), true);
      } else {
