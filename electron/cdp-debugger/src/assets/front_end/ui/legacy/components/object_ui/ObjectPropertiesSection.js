...
export class ObjectPropertiesSection extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  static defaultObjectPropertiesSection(object, linkifier, skipProto, readOnly) {
    ...
    const propertyValue = ObjectPropertiesSection.createPropertyValue(object, false, true);
  ...
  static assignWebIDLMetadata(value, properties) {
    ...
    const webIdlType = isInstance ? domPinnedProperties[value.className] : void 0;
    if (webIdlType) {
      value.webIdl = { info: webIdlType, state: /* @__PURE__ */ new Map() };
    } else {
      return;
    }
  ...
  static createPropertyValue(value, wasThrown, showPreview, parentElement, linkifier) {
    ...
    if (type === "object" && subtype === "internal#location") {
      ...
      if (value.preview && showPreview) {
        ...
        previewFormatter.appendObjectPreview(valueElement, value.preview, false);
...
export var ObjectPropertiesMode = /* @__PURE__ */ ((ObjectPropertiesMode2) => {
  ...
  ObjectPropertiesMode2[ObjectPropertiesMode2["OwnAndInternalAndInherited"] = 1] = "OwnAndInternalAndInherited";
...
export class ObjectPropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static async populate(treeElement, value, skipProto, skipGettersAndSetters, linkifier, emptyPlaceholder, propertiesMode = 1 /* OwnAndInternalAndInherited */, extraProperties, targetValue) {
    ...
    switch (propertiesMode) {
      case 0 /* All */:
        ({ properties } = await value.getAllProperties(false, true));
        break;
      case 1 /* OwnAndInternalAndInherited */:
        ({ properties, internalProperties } = await SDK.RemoteObject.RemoteObject.loadFromObjectPerProto(value, true));
        break;
    }
    treeElement.removeChildren();
    if (!properties) {
      ...
      for (const property of properties) {
        if (property.webIdl) {
          ...
          if (!parentRules && !propertyRules) {
            ...
            property.webIdl.applicable = !propertyRules || propertyRules?.some((rule) => parentState.get(rule.when) === rule.is);
          }
        }
      }
    }
  ...
  async onpopulate() {
    ...
    if (targetValue) {
      await ObjectPropertyTreeElement.populate(this, propertyValue, skipProto, false, this.linkifier, void 0, void 0, void 0, targetValue);
...
export class ArrayGroupingTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static async populateAsFragment(treeNode, object, fromIndex, toIndex, linkifier) {
    ...
    const allProperties = await arrayFragment.getAllProperties(false, true);
  ...
  static async populateNonIndexProperties(treeNode, object, linkifier) {
    const { properties, internalProperties } = await SDK.RemoteObject.RemoteObject.loadFromObjectPerProto(object, true, true);
    if (!properties) {
      return;
    }
...
const cachedResultMap = /* @__PURE__ */ new Map();
export class ObjectPropertiesSectionsTreeExpandController {
  expandedProperties;
  constructor(treeOutline) {
    this.expandedProperties = /* @__PURE__ */ new Set();
