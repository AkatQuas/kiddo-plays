...
export class BezierEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  updateUI() {
    const labelText = this.selectedCategory ? this.selectedCategory.presets[this.selectedCategory.presetIndex].name : this.bezierInternal.asCSSText().replace(/\s(-\d\.\d)/g, "$1");
  ...
  dragStart(event) {
    ...
    const firstControlPointIsCloser = this.controlPosition.distanceTo(this.bezierInternal.controlPoints[0]) < this.controlPosition.distanceTo(this.bezierInternal.controlPoints[1]);
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["BezierChanged"] = "BezierChanged";
  return Events2;
})(Events || {});
