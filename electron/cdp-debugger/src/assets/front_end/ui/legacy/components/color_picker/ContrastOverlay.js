...
export class ContrastRatioLineBuilder {
  ...
  drawContrastRatioLine(width, height, level) {
    ...
    const desiredLuminance = isAPCA ? Common.ColorUtils.desiredLuminanceAPCA(bgLuminance, requiredContrast, fgIsLighter) : Common.Color.Color.desiredLuminance(bgLuminance, requiredContrast, fgIsLighter);
    ...
    let candidateLuminance = (candidateHSVA2) => {
      return Common.ColorUtils.luminance(Common.ColorUtils.blendColors(Common.Color.Color.fromHSVA(candidateHSVA2).rgba(), bgRGBA));
    };
    if (Root.Runtime.experiments.isEnabled("APCA")) {
      candidateLuminance = (candidateHSVA2) => {
        return Common.ColorUtils.luminanceAPCA(Common.ColorUtils.blendColors(Common.Color.Color.fromHSVA(candidateHSVA2).rgba(), bgRGBA));
      };
    }
    let s;
    for (s = 0; s < 1 + dS; s += dS) {
