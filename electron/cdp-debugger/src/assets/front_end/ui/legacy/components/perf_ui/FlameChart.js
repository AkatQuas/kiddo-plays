...
export class FlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(dataProvider, flameChartDelegate, groupExpansionSetting) {
    ...
    this.textWidth = /* @__PURE__ */ new Map();
  ...
  revealEntry(entryIndex) {
    ...
    if (timeLeft > entryEndTime) {
      ...
      this.windowChanged(timeLeft - delta, timeRight - delta, true);
  ...
  onMouseMove(event) {
    ...
    if (this.coordinatesToGroupIndex(mouseEvent.offsetX, mouseEvent.offsetY, true) >= 0) {
      this.hideHighlight();
  ...
  updateHighlight() {
    ...
    if (entryIndex === -1) {
      this.hideHighlight();
      const group = this.coordinatesToGroupIndex(this.lastMouseOffsetX, this.lastMouseOffsetY, false);
      if (group >= 0 && this.rawTimelineData && this.rawTimelineData.groups && this.rawTimelineData.groups[group].selectable) {
        this.viewportElement.style.cursor = "pointer";
      } else {
  ...
  onClick(event) {
    ...
    this.selectGroup(this.coordinatesToGroupIndex(mouseEvent.offsetX, mouseEvent.offsetY, false));
    this.toggleGroupExpand(this.coordinatesToGroupIndex(mouseEvent.offsetX, mouseEvent.offsetY, true));
    const timelineData = this.timelineData();
  ...
  toggleGroupExpand(groupIndex) {
    ...
    this.expandGroup(groupIndex, !this.rawTimelineData.groups[groupIndex].expanded);
  }
  ...
  handleKeyboardGroupNavigation(event) {
    ...
    if (keyboardEvent.code === "ArrowUp") {
      ...
      if (this.keyboardFocusedGroup >= 0) {
        this.expandGroup(this.keyboardFocusedGroup, false);
        handled = true;
      }
    } else if (keyboardEvent.code === "ArrowRight") {
      if (this.keyboardFocusedGroup >= 0) {
        this.expandGroup(this.keyboardFocusedGroup, true);
  ...
  selectPreviousGroup() {
    ...
    const groupIndexToSelect = this.getGroupIndexToSelect(-1);
  ...
  coordinatesToEntryIndex(x, y) {
    ...
    const indexOnLevel = Math.max(Platform.ArrayUtilities.upperBound(entriesOnLevel, cursorTime, (time, entryIndex2) => time - entryStartTimes[entryIndex2]) - 1, 0);
  ...
  draw() {
    ...
    for (let level = minVisibleBarLevel; level < this.dataProvider.maxStackDepth(); ++level) {
      ...
      const rightIndexOnLevel = Platform.ArrayUtilities.lowerBound(levelIndexes, this.chartViewport.windowRightTime(), (time, entryIndex) => time - entryStartTimes[entryIndex]) - 1;
    ...
    if (this.useWebGL) {
      ...
      this.forEachGroupInViewport((offset, index, group, isFirst, groupHeight) => {
        if (this.isGroupFocused(index)) {
          context.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue("--selected-group-background", this.contentElement);
    ...
    for (let m = markerIndices.length - 1; m >= 0; --m) {
      ...
      const padding = 4;
      const width2 = Math.ceil(UI.UIUtils.measureTextWidth(context, title)) + 2 * padding;
      lastMarkerX = x + width2 + 1;
      lastMarkerLevel = level;
      this.markerPositions.set(entryIndex, { x, width: width2 });
      context.fillStyle = this.dataProvider.entryColor(entryIndex);
      context.fillRect(x, y, width2, h - 1);
  ...
  setupGLGeometry() {
    ...
    let vertex = 0;
    const parsedColorCache = /* @__PURE__ */ new Map();
    const colors = [];
    ...
    if (this.colorBuffer && this.aVertexColor) {
      ...
      gl.vertexAttribPointer(this.aVertexColor, 1, colorIndexType, true, 0, 0);
    }
  }
  drawGL() {
    const gl = this.canvasGL.getContext("webgl");
    ...
    const viewportScale = [2 / this.boundarySpan(), -2 * window.devicePixelRatio / this.canvasGL.height];
  ...
  drawGroupHeaders(width, height) {
    ...
    this.forEachGroupInViewport((offset, index, group) => {
      ...
      if (this.isGroupCollapsible(index) && !group.expanded || group.style.shareHeaderLine) {
        const width2 = this.labelWidthForGroup(context, group) + 2;
        if (this.isGroupFocused(index)) {
          context.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue("--selected-group-background", this.contentElement);
        } else {
        ...
        context.fillRect(this.headerLeftPadding - this.headerLabelXPadding, offset + this.headerLabelYPadding, width2, group.style.height - 2 * this.headerLabelYPadding);
      }
  ...
  labelWidthForGroup(context, group) {
    return UI.UIUtils.measureTextWidth(context, group.name) + this.expansionArrowIndent * (group.style.nestingLevel + 1) + 2 * this.headerLabelXPadding;
  }
  drawCollapsedOverviewForGroup(group, y, endLevel) {
    ...
    for (let level = group.startLevel; level < endLevel; ++level) {
      const levelIndexes = this.timelineLevels ? this.timelineLevels[level] : [];
      const rightIndexOnLevel = Platform.ArrayUtilities.lowerBound(levelIndexes, timeWindowRight, (time, entryIndex) => time - entryStartTimes[entryIndex]) - 1;
      let lastDrawOffset = Infinity;
      for (let entryIndexOnLevel = rightIndexOnLevel; entryIndexOnLevel >= 0; --entryIndexOnLevel) {
        const entryIndex = levelIndexes[entryIndexOnLevel];
        const entryStartTime = entryStartTimes[entryIndex];
  ...
  updateLevelPositions() {
    ...
    for (level = 0; level < lastGroupLevel; ++level) {
      ...
      while (groupIndex < groups.length - 1 && level === groups[groupIndex + 1].startLevel) {
        ...
        groupStack.push({ nestingLevel: style.nestingLevel, visible });
      ...
      if (groupIndex >= 0) {
        ...
        height = isFirstOnLevel && !styleB.shareHeaderLine || styleB.collapsible && !group.expanded ? styleB.height : styleB.itemsHeight || this.barHeight;
      } else {
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["CanvasFocused"] = "CanvasFocused";
  Events2["EntryInvoked"] = "EntryInvoked";
  Events2["EntrySelected"] = "EntrySelected";
  Events2["EntryHighlighted"] = "EntryHighlighted";
  return Events2;
})(Events || {});
