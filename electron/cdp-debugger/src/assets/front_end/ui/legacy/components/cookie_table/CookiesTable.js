...
export class CookiesTable extends UI.Widget.VBox {
  ...
  setCookies(cookies, cookieToBlockedReasons) {
    this.setCookieFolders([{ cookies, folderName: null }], cookieToBlockedReasons);
  }
  ...
  isSameCookie(cookieA, cookieB) {
    return cookieB !== null && cookieB !== void 0 && cookieB.name() === cookieA.name() && cookieB.domain() === cookieA.domain() && cookieB.path() === cookieA.path();
  }
  rebuildTable() {
  ...
  createGridNode(cookie) {
    ...
    if (cookie.maxAge()) {
      ...
      data[SDK.Cookie.Attributes.Expires] = cookie.type() === SDK.Cookie.Type.Request ? i18nString(UIStrings.na) : expiresSessionValue();
    }
  ...
  createCookieFromData(data) {
    ...
    if (SDK.Cookie.Attributes.SourcePort in data) {
      cookie.addAttribute(SDK.Cookie.Attributes.SourcePort, Number.parseInt(data[SDK.Cookie.Attributes.SourcePort], 10) || void 0);
    }
  ...
  isValidCookieData(data) {
    return (Boolean(data.name) || Boolean(data.value)) && this.isValidDomain(data.domain) && this.isValidPath(data.path) && this.isValidDate(data.expires);
  }
