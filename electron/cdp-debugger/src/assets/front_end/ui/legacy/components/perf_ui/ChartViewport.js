...
export class ChartViewport extends UI.Widget.VBox {
  ...
  onMouseWheel(e) {
    const wheelEvent = e;
    const doZoomInstead = wheelEvent.shiftKey !== (Common.Settings.Settings.instance().moduleSetting("flamechartMouseWheelAction").get() === "zoom");
    ...
    if (panVertically) {
      ...
      this.handlePanGesture(wheelEvent.deltaX, true);
    } else {
  ...
  handleZoomPanKeys(e) {
    ...
    switch (keyboardEvent.code) {
      case "KeyA":
        this.handlePanGesture(-panOffset, true);
        break;
      case "KeyD":
        this.handlePanGesture(panOffset, true);
        break;
  ...
  handleZoomGesture(zoom) {
    ...
    this.requestWindowTimes(bounds, true);
  }
