...
export class Toolbar {
  ...
  constructor(className, parentElement) {
    ...
    this.enabled = true;
    this.shadowRoot = Utils.createShadowRootWithCoreStyles(this.element, { cssFile: toolbarStyles, delegatesFocus: void 0 });
  ...
  static createActionButton(action, options = TOOLBAR_BUTTON_DEFAULT_OPTIONS) {
    ...
    return button;
    function makeButton() {
      const button2 = new ToolbarButton(action.title(), action.icon());
      if (action.title()) {
  ...
  hideSeparatorDupes() {
    ...
    this.element.classList.toggle("hidden", lastSeparator !== null && lastSeparator !== void 0 && lastSeparator.visible() && !nonSeparatorVisible);
  }
  async appendItemsAtLocation(location) {
    ...
    const items = await Promise.all(filtered.map((extension) => {
      ...
      if (actionId) {
        return Toolbar.createActionButtonForId(actionId, { showLabel: Boolean(showLabel), userActionCode: void 0 });
      }
      if (!loadItem) {
...
export var ToolbarItemWithCompactLayoutEvents = /* @__PURE__ */ ((ToolbarItemWithCompactLayoutEvents2) => {
  ToolbarItemWithCompactLayoutEvents2["CompactLayoutUpdated"] = "CompactLayoutUpdated";
...
((ToolbarInput2) => {
  let Event;
  ((Event2) => {
    Event2["TextChanged"] = "TextChanged";
    Event2["EnterPressed"] = "EnterPressed";
...
function getRegisteredToolbarItems() {
  return registeredToolbarItems.filter((item) => Root.Runtime.Runtime.isDescriptorEnabled({ experiment: void 0, condition: item.condition }));
}
