...
export class TextEditor extends HTMLElement {
  ...
  revealPosition(selection, highlight = true) {
    ...
    const effects = [];
    if (highlight) {
      effects.push(view.state.field(highlightState, false) ? setHighlightLine.of(line.from) : CodeMirror.StateEffect.appendConfig.of(highlightState.init(() => highlightDeco(line.from))));
    }
...
function highlightDeco(position) {
  ...
  return { deco, id: Math.floor(Math.random() * 1048575) };
}
