...
export function buildDiffRows(diff) {
  ...
  function createModifyRows(before, after) {
    const internalDiff = Diff.Diff.DiffWrapper.charDiff(before, after, true);
...
class DiffRenderer {
  ...
  #renderRowContent(row) {
    ...
    for (const token of row.tokens) {
      ...
      content.push(token.className ? LitHtml.html`<span class=${token.className}>${tokenContent}</span>` : LitHtml.html`${tokenContent}`);
