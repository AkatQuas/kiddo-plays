...
class FilePathIndex {
  ...
  similarFiles(networkPath) {
    ...
    return this.reversedIndex.words(longestCommonPrefix).map((encodedPath2) => this.encoder.decode(Platform.StringUtilities.reverse(encodedPath2)));
  }
}
class FolderIndex {
  encoder;
  ...
  constructor(encoder) {
    ...
    this.folderCount = /* @__PURE__ */ new Map();
  }
  addFolder(path) {
    if (path.endsWith("/")) {
