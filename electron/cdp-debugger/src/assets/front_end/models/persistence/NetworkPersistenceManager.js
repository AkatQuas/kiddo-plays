...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  rawPathFromUrl(url, ignoreInactive) {
    ...
    function encodeEncodedPathToLocalPathParts(encodedPath2) {
      const encodedParts = [];
      for (const pathPart of fileNamePartsFromEncodedPath(encodedPath2)) {
        if (!pathPart) {
          continue;
        }
    ...
    function fileNamePartsFromEncodedPath(encodedPath2) {
      encodedPath2 = Common.ParsedURL.ParsedURL.urlWithoutHash(encodedPath2);
      const queryIndex = encodedPath2.indexOf("?");
      if (queryIndex === -1) {
        return encodedPath2.split("/");
      }
      if (queryIndex === 0) {
        return [encodedPath2];
      }
      const endSection = encodedPath2.substr(queryIndex);
      const parts = encodedPath2.substr(0, encodedPath2.length - endSection.length).split("/");
  ...
  getHeadersUISourceCodeFromUrl(url) {
    const fileUrlFromRequest = this.fileUrlFromNetworkUrl(url, true);
  ...
  async getOrCreateHeadersUISourceCodeFromUrl(url) {
    ...
    if (!uiSourceCode && this.projectInternal) {
      const encodedFilePath = this.encodedPathFromUrl(url, true);
  ...
  canSaveUISourceCodeForOverrides(uiSourceCode) {
    return this.activeInternal && uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network && !this.bindings.has(uiSourceCode) && !this.savingForOverrides.has(uiSourceCode);
  }
  ...
  async #innerUpdateInterceptionPatterns() {
    ...
    for (const uiSourceCode of this.projectInternal.uiSourceCodes()) {
      ...
      if (Root.Runtime.experiments.isEnabled(Root.Runtime.ExperimentName.HEADER_OVERRIDES) && uiSourceCode.name() === HEADERS_FILENAME) {
    ...
    return SDK.NetworkManager.MultitargetNetworkManager.instance().setInterceptionHandlerForPatterns(Array.from(patterns).map((pattern) => ({ urlPattern: pattern, requestStage: Protocol.Fetch.RequestStage.Response })), this.interceptionHandlerBound);
  }
  ...
  async interceptionHandler(interceptedRequest) {
    ...
    if (fileSystemUISourceCode) {
      ...
      if (!responseBody.error && responseBody.content) {
        void interceptedRequest.continueRequestWithContent(new Blob([responseBody.content], { type: mimeType }), true, responseHeaders);
      }
    }
  }
}
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["ProjectChanged"] = "ProjectChanged";
  return Events2;
})(Events || {});
