...
export class DebuggerWorkspaceBinding {
  ...
  constructor(targetManager, workspace) {
    ...
    this.pluginManager = Root.Runtime.experiments.isEnabled("wasmDWARFDebugging") ? new DebuggerLanguagePluginManager(targetManager, workspace, this) : null;
  }
...
class ModelData {
  ...
  uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber = 0) {
    ...
    locations = locations.length ? locations : this.resourceMapping.uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber);
    locations = locations.length ? locations : ResourceMapping.instance().uiLocationToJSLocations(uiSourceCode, lineNumber, columnNumber);
