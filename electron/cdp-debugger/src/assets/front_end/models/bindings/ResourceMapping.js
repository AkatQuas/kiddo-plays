...
const styleSheetOffsetMap = /* @__PURE__ */ new WeakMap();
const scriptOffsetMap = /* @__PURE__ */ new WeakMap();
...
export class ResourceMapping {
  ...
  static removeInstance() {
    resourceMappingInstance = void 0;
  }
  ...
  cssLocationToUILocation(cssLocation) {
    ...
    const offset = styleSheetOffsetMap.get(header) || TextUtils.TextRange.TextRange.createFromLocation(header.startLine, header.startColumn);
  ...
  jsLocationToUILocation(jsLocation) {
    ...
    const offset = scriptOffsetMap.get(script) || TextUtils.TextRange.TextRange.createFromLocation(script.lineOffset, script.columnOffset);
  ...
  uiLocationToJSLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    for (const script of debuggerModel.scripts()) {
      ...
      const { startLine, startColumn } = scriptOffsetMap.get(script) || TextUtils.TextRange.TextRange.createFromLocation(script.lineOffset, script.columnOffset);
...
class ModelInfo {
  ...
  constructor(workspace, resourceTreeModel) {
    ...
    this.project = new ContentProviderBasedProject(workspace, "resources:" + target.id(), Workspace.Workspace.projectTypes.Network, "", false);
  ...
  acceptsResource(resource) {
    ...
    if (resourceType !== Common.ResourceType.resourceTypes.Image && resourceType !== Common.ResourceType.resourceTypes.Font && resourceType !== Common.ResourceType.resourceTypes.Document && resourceType !== Common.ResourceType.resourceTypes.Manifest) {
      return false;
    }
    ...
    if ((resourceType === Common.ResourceType.resourceTypes.Image || resourceType === Common.ResourceType.resourceTypes.Font) && resource.contentURL().startsWith("data:")) {
      return false;
    }
    return true;
  }
...
class Binding {
  ...
  async innerStyleSheetChanged(content) {
    ...
    for (const data of this.#edits) {
      ...
      const startLocation = styleSheetOffsetMap.get(stylesheet) || TextUtils.TextRange.TextRange.createFromLocation(stylesheet.startLine, stylesheet.startColumn);
      ...
      for (const script of scripts) {
        const scriptOffset = scriptOffsetMap.get(script) || TextUtils.TextRange.TextRange.createFromLocation(script.lineOffset, script.columnOffset);
      ...
      for (const style of styles) {
        const styleOffset = styleSheetOffsetMap.get(style) || TextUtils.TextRange.TextRange.createFromLocation(style.startLine, style.startColumn);
