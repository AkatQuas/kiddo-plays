...
export class CompilerScriptMapping {
  ...
  constructor(debuggerModel, workspace, debuggerWorkspaceBinding) {
    ...
    this.#regularProject = new ContentProviderBasedProject(workspace, "jsSourceMaps::" + target.id(), Workspace.Workspace.projectTypes.Network, "", false);
    this.#contentScriptsProject = new ContentProviderBasedProject(workspace, "jsSourceMaps:extensions:" + target.id(), Workspace.Workspace.projectTypes.ContentScripts, "", false);
    ...
    this.#regularBindings = /* @__PURE__ */ new Map();
    this.#contentScriptsBindings = /* @__PURE__ */ new Map();
    this.#stubUISourceCodes = /* @__PURE__ */ new Map();
    this.#stubProject = new ContentProviderBasedProject(workspace, "jsSourceMaps:stub:" + target.id(), Workspace.Workspace.projectTypes.Service, "", true);
    this.#eventListeners = [
  ...
  uiSourceCodeForURL(url, isContentScript) {
    return isContentScript ? this.#contentScriptsProject.uiSourceCodeForURL(url) : this.#regularProject.uiSourceCodeForURL(url);
  }
  rawLocationToUILocation(rawLocation) {
    ...
    const uiSourceCode = script.isContentScript() ? this.#contentScriptsProject.uiSourceCodeForURL(entry.sourceURL) : this.#regularProject.uiSourceCodeForURL(entry.sourceURL);
    if (!uiSourceCode) {
      return null;
    }
