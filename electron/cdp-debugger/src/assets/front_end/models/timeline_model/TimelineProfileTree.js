...
export class TopDownNode extends Node {
  ...
  buildChildren() {
    ...
    const instantEventCallback = root.doNotAggregate ? onInstantEvent : void 0;
    const eventIdCallback = root.doNotAggregate ? void 0 : _eventId;
    ...
    function onStartEvent(e) {
      ...
      const actualEndTime = e.endTime !== void 0 ? Math.min(e.endTime, endTime) : endTime;
...
export class BottomUpRootNode extends Node {
  ...
  ungrouppedTopNodes() {
    ...
    TimelineModelImpl.forEachEvent(this.events, onStartEvent, onEndEvent, void 0, startTime, endTime, this.filter);
    function onStartEvent(e) {
      const actualEndTime = e.endTime !== void 0 ? Math.min(e.endTime, endTime) : endTime;
...
export class BottomUpNode extends Node {
  ...
  children() {
    ...
    const self = this;
    TimelineModelImpl.forEachEvent(this.root.events, onStartEvent, onEndEvent, void 0, startTime, endTime, this.root.filter);
    function onStartEvent(e) {
      const actualEndTime = e.endTime !== void 0 ? Math.min(e.endTime, endTime) : endTime;
      const duration = actualEndTime - Math.max(e.startTime, startTime);
      if (duration < 0) {
...
export function _eventId(event) {
  ...
  const name = TimelineJSProfileProcessor.isNativeRuntimeFrame(frame) ? TimelineJSProfileProcessor.nativeGroup(functionName) || functionName : `${functionName}:${frame["lineNumber"]}:${frame["columnNumber"]}`;
