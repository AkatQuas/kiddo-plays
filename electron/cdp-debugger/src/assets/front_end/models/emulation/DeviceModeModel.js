...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#deviceScaleFactorSettingInternal = Common.Settings.Settings.instance().createSetting("emulation.deviceScaleFactor", 0);
    this.#deviceScaleFactorSettingInternal.addChangeListener(this.deviceScaleFactorSettingChanged, this);
    this.#deviceOutlineSettingInternal = Common.Settings.Settings.instance().moduleSetting("emulation.showDeviceOutline");
  ...
  emulate(type, device, mode, scale) {
    ...
    if (type === Type.Device && device && mode) {
      ...
      if (this.#initialized) {
        ...
        this.#scaleSettingInternal.set(scale || this.calculateFitScale(orientation.width, orientation.height, this.currentOutline(), this.currentInsets()));
      }
    } else {
      this.#deviceInternal = null;
      this.#modeInternal = null;
  ...
  calculateAndEmulate(resetPageScaleFactor) {
    ...
    if (this.#typeInternal === Type.Device && this.#deviceInternal && this.#modeInternal) {
      ...
      this.applyDeviceMetrics(new UI.Geometry.Size(screenWidth, screenHeight), new Insets(0, 0, 0, 0), new Insets(0, 0, 0, 0), this.#scaleSettingInternal.get(), this.#deviceScaleFactorSettingInternal.get() || defaultDeviceScaleFactor, mobile, screenHeight >= screenWidth ? Protocol.Emulation.ScreenOrientationType.PortraitPrimary : Protocol.Emulation.ScreenOrientationType.LandscapePrimary, resetPageScaleFactor);
...
export var UA = /* @__PURE__ */ ((UA2) => {
  UA2["Mobile"] = "Mobile";
  UA2["MobileNoTouch"] = "Mobile (no touch)";
  UA2["Desktop"] = "Desktop";
  UA2["DesktopTouch"] = "Desktop (touch)";
  return UA2;
})(UA || {});
