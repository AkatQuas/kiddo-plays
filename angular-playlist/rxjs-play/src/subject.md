...
[original](https://rxjs-dev.firebaseapp.com/guide/subject)
...
1. **The multicasted Observable is connected**
1. The `next` value `0` is delivered to the first Observer
1. Second Observer subscribes to the multicasted Observable
1. The `next` value `1` is delivered to the first Observer
1. The `next` value `1` is delivered to the second Observer
1. First Observer unsubscribes from the multicasted Observable
1. The `next` value `2` is delivered to the second Observer
...
To achieve that with explicit calls to `connect()`, codes are like this:
